...
在已经获得稳定正向 `Delta ADE` 之后，下一阶段优化重点应从“解决语言塌缩”转向“提升稳定性并改善 FDE”。当前已将默认训练配置调整为折中版本：设置 `rank_loss_weight = 0.6`、`rank_margin = 0.015`，启用 `grad_clip_norm = 1.0`，并加入轻量终点约束 `fde_loss_weight = 0.05`。这组参数的目标是把 `Delta ADE` 拉回 `0.02+` 附近，同时减少 Dual-Head 排序约束对最终落点的负面影响。

后续建议采用逐步消融方式进行训练，而不是一次性修改多个变量。第一步运行当前折中配置，观察 `Delta ADE` 是否保持大于 `0.02`，以及 `Delta FDE` 是否接近 0。若 `FDE` 仍明显落后，可尝试将 `fde_loss_weight` 提升到 `0.1` 或 `0.2`；若 `Delta ADE` 明显下降，则说明终点约束压过了语言收益，应回退到更强的 rank 配置或降低终点权重。
...
| Delta-strong | 0.7 | 0.02 | 0.05 | 优先保持 Delta ADE |

每个候选配置都应至少运行 `42/43/44` 三个随机种子，并使用 `language_effect_report.json` 和 `benchmark_report.json` 同时判断。建议通过标准为：`delta_ade.mean > 0.02`，`delta_ade.std < 0.008`，三个 seed 的 `delta_ade` 均为正，同时 `delta_fde.mean` 尽量接近 0 或为正。
...
本文围绕 doScenes Challenge 的 Language+History 任务，提出了一套面向语言增益优化的 Dual-Head 轨迹预测框架。该方法以 DistilBERT、GRU 和 cross-attention 为基础，通过非空指令重采样、文本编码器部分解冻以及排序损失三类策略，成功解决了单头模型中常见的语言塌缩问题。在当前实验设置下，模型在单次实验中获得 `Delta ADE = 0.030314`，在三个随机种子上取得平均 `Delta ADE = 0.025544 +/- 0.005406`，并完成了符合提交协议的结果导出与校验。进一步地，本文也明确讨论了语言条件预测的伦理边界：语言可以提供意图线索，但不能覆盖交通法规、安全约束和下游规划检查。整体来看，该方法证明了：即使不依赖超大规模生成模型，只要在结构设计和优化目标上显式面向语言增益，轻量模型同样能够在 doScenes 任务中稳定发挥语言条件价值；而后续若要走向更可靠的系统，还必须同时优化指标、安全和鲁棒性。
