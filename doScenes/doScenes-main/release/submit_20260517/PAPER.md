...
实验结果表明，单纯的轨迹回归目标会使模型容易退化为“只依赖历史轨迹，不真正利用文本”，而 Dual-Head 设计配合非空指令重采样后，能够稳定恢复语言增益。在当前版本实验中（以下全部为 meters 口径），模型单次运行取得 `ADE_instruction = 3.474533`、`ADE_baseline = 3.955950`、`Delta ADE = 0.481417`；在 `42/43/44` 三个随机种子上，平均 `Delta ADE = 0.404754 +/- 0.054824`。此外，最终导出的提交文件已经通过本地提交前校验，满足 `sample_token,instruction,x1,y1,...,x12,y12` 的官方格式约束。整体来看，该方法在工程上可复现、在指标上有效、在提交协议上可落地，为 doScenes 语言条件轨迹预测提供了一条轻量但实用的实现路径。
...
最关键的结论来自 `has_instruction` 子集。该子集上的 `Delta ADE = +0.380370 m`，说明模型已经能够在真实语言条件样本上利用文本信息改善轨迹预测，而不再只是形式上“接入了文本”。
...
从表中可以看到，三次实验的 `Delta ADE` 均保持正值，说明语言增益具备稳定性，而不是偶发结果。与此同时，`Delta FDE` 也转为正值，说明终点误差在当前配置下已较早期版本明显改善。
...
本文围绕 doScenes Challenge 的 Language+History 任务，提出了一套面向语言增益优化的 Dual-Head 轨迹预测框架。该方法以 DistilBERT、GRU 和 cross-attention 为基础，通过非空指令重采样、文本编码器部分解冻以及排序损失三类策略，成功解决了单头模型中常见的语言塌缩问题。在当前实验设置下（meters），模型在单次实验中获得 `Delta ADE = 0.481417 m`，在三个随机种子上取得平均 `Delta ADE = 0.404754 +/- 0.054824 m`，并完成了符合提交协议的结果导出与校验。进一步地，本文也明确讨论了语言条件预测的伦理边界：语言可以提供意图线索，但不能覆盖交通法规、安全约束和下游规划检查。整体来看，该方法证明了：即使不依赖超大规模生成模型，只要在结构设计和优化目标上显式面向语言增益，轻量模型同样能够在 doScenes 任务中稳定发挥语言条件价值；而后续若要走向更可靠的系统，还必须同时优化指标、安全和鲁棒性。
