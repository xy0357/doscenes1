...
In the final submission setup, the official instruction-conditioned evaluation reports `ADE_instruction = 3.435528`, `ADE_baseline = 3.998661`, `Delta ADE = 0.563133`, `FDE_instruction = 8.289304`, and `Delta FDE = 0.359455`. The corrected submission files also satisfy official protocol constraints, including exact row count and scene-token matching.
...
4. Lower final instruction ADE under the official instruction-conditioned evaluation protocol together with corrected protocol-compliant submission outputs.
...
The final language report is generated under the official instruction-conditioned evaluation protocol, so no separate `empty_instruction` subset appears in the packaged submission report.

### 7.3 Submission-scope note

This final package is centered on the `configs/ade_query_residual.json` checkpoint and reports the exact metrics required by the challenge submission form. Older multi-seed benchmark tables are intentionally excluded from the main results here so that the final paper does not mix statistics from different architectures or different evaluation scopes.
...
The current model improves both average trajectory quality and endpoint quality under the official report scope: `FDE_instruction = 8.289304` is better than the baseline `8.648759`. This is encouraging because the improvement is not achieved by sacrificing endpoint accuracy.

At the same time, an `8 m+` endpoint error still leaves meaningful room for improvement. Future tuning should therefore continue to strengthen endpoint modeling while preserving the current ADE advantage.
